...
namespace Monsajem_Incs.HttpService
{
        ...
        IComparable<Request>
    {
        ...
        {
            Service = new Task(() => {
                if (Stoped == true) TreadeExtentions.StopThreade();
            });
        }
        ...
        public static void Ready(HttpListenerContext MyContext)
        {
            ...
            if (Request.RawUrl.Length > 1)
            {
                ...
                while (End != -1)
                {
                    Insert(ref Steps, Request.Url.AbsolutePath.Substring(Position, End - Position));
                    Position = End + 1;
                    End = Request.Url.AbsolutePath.IndexOf("/", Position);
                }
                if (Position != Request.Url.AbsolutePath.Length)
                    Insert(ref Steps, Request.Url.AbsolutePath.Substring(Position, Request.Url.AbsolutePath.Length - Position));
            }
        }
        ...
        public static void Fetch()
        {
            ...
            lock(FetchedRequests)
                BinaryInsert(ref FetchedRequests, rq);
            ...
            {
                lock (FetchedRequests)
                    BinaryDelete(ref FetchedRequests, rq);
        ...
        internal void Continue(HttpListenerContext MyContext)
        {
            if (MyContext.Request.UserHostAddress != this.Address)
            {
                this.Stoped=true;
                lock (FetchedRequests)
                    BinaryDelete(ref FetchedRequests, this);
                TreadeExtentions.StopThreade();
            }
