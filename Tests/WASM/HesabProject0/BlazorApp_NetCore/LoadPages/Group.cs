                ...
                c.SetView<GroupView_html>((i) =>
                {
                    i.MakeHolder = (Table) =>
                    {
                        ...
                        if (PartTable != null)
                        {
                            ...
                            var Holder = Data.Groups[(string)PartTable.HolderTable.Key].Value;
                            if (IsUser == false)
                            {
                               
                                {
                                    var btn = new Monsajem_Incs.Resources.Base.Html.input_button_html().Main;
                                    btn.Value = "افزودن گروه";
                                    btn.OnClick += (c1, c2) =>
                                    {
                                        Table.ShowInsert();
                                    };
                                ...
                                {
                                    var btn = new Monsajem_Incs.Resources.Base.Html.input_button_html().Main;
                                    btn.Value = "افزودن کالا";
                                    btn.OnClick += (c1, c2) =>
                                    {
                                        Holder.ProductChilds.ShowInsert();
                                    };
                                    Div.AppendChild(btn);
                                }
                            }
                            
                            if (Holder.ProductChilds.Length > 0)
                                Div.AppendChild(Holder.ProductChilds.MakeShowView());
                        }
                    ...
                    i.FillView = (i) =>
                    {
                        ...
                        {
                            i.View.Link.Remove();
                            i.View.btn_showIncludes.OnClick += (c1, c2) => i.Value.GroupChilds.ShowItems();
