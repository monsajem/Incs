...
namespace Calculate_wall
{
    partial class Calculator
    {
        ...
        {
            MakeRateOnGrowing(Values,
                              (c)=>GetInfo(c).RSI_14,
                              (c)=>GetInfo(c.Info).Rate_RSI_14 = c.Rate);

            MakeRateOnGrowing(Values,
                              (c) => GetInfo(c).RSI_100,
                              (c) => GetInfo(c.Info).Rate_RSI_100 = c.Rate);

            MakeRateOnGrowing(Values,
                              (c) => GetInfo(c).MFI_14,
                              (c) => GetInfo(c.Info).Rate_MFI_14 = c.Rate);

            MakeRateOnGrowing(Values,
                              (c) => GetInfo(c).MFI_100,
                              (c) => GetInfo(c.Info).Rate_MFI_100 = c.Rate);

            MakeRateOnGrowing(Values,
                  (c) => GetInfo(c).CRSI,
                  (c) => GetInfo(c.Info).Rate_CRSI = c.Rate);

            MakeRateOnGrowing(Values,
                  (c) => GetInfo(c).DEMA_Cross,
                  (c) => GetInfo(c.Info).Rate_DEMA_Cross = c.Rate);
        }
